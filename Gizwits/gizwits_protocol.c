...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagsay_flag)
    {
        info->event[info->num] = EVENT_say_flag;
        info->num++;
        dataPoints->valuesay_flag = gizStandardDecompressionValue(say_flag_BYTEOFFSET,say_flag_BITOFFSET,say_flag_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuesay_flag != cur->valuesay_flag)
    {
        GIZWITS_LOG("valuesay_flag Changed\n");
        ret = 1;
    }
    if(last->valueover_weight_flag != cur->valueover_weight_flag)
    {
        GIZWITS_LOG("valueover_weight_flag Changed\n");
        ret = 1;
    }

    if(last->valueweight_data != cur->valueweight_data)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueweight_data Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(say_flag_BYTEOFFSET,say_flag_BITOFFSET,say_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valuesay_flag);
    gizStandardCompressValue(over_weight_flag_BYTEOFFSET,over_weight_flag_BITOFFSET,over_weight_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueover_weight_flag);
    ...
    devStatusPtr->valueweight_data = exchangeBytes(gizY2X(weight_data_RATIO,  weight_data_ADDITION, dataPoints->valueweight_data)); 
